...
func (c *BaseController) getNotes(w http.ResponseWriter, r *http.Request) {
	userID := getUserID(w, r)
	if userID == nil {
		return
	}
	...
	tx, _ := c.stor.Tx(r.Context())
	defer tx.Commit()
	result, err := c.notesRepo.ReadMany(r.Context(), tx, *userID, startID, batchSize)
	if err != nil {
		defer tx.Rollback()
		w.WriteHeader(http.StatusInternalServerError)
		msg := fmt.Errorf("failed to get notes: %v", err).Error()
		w.Write([]byte(msg))
		return
	}
	payload, err := json.Marshal(result)
...
func (c *BaseController) getCards(w http.ResponseWriter, r *http.Request) {
	userID := getUserID(w, r)
	if userID == nil {
		return
	}
	...
	tx, _ := c.stor.Tx(r.Context())
	defer tx.Commit()
	result, err := c.cardsRepo.ReadMany(r.Context(), tx, *userID, startID, batchSize)
	if err != nil {
		defer tx.Rollback()
		w.WriteHeader(http.StatusInternalServerError)
		msg := fmt.Errorf("failed to get cards: %v", err).Error()
		w.Write([]byte(msg))
		return
	}
	payload, err := json.Marshal(result)
...
func (c *BaseController) deleteFile(w http.ResponseWriter, r *http.Request) {
	userID := getUserID(w, r)
	if userID == nil {
		return
	}
	rowIDRaw := chi.URLParam(r, "fileID")
	rowID, err := strconv.Atoi(rowIDRaw)
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		msg := fmt.Errorf("not a valid row ID: %v", err).Error()
	...
	tx, _ := c.stor.Tx(r.Context())
	defer tx.Commit()
	if err := c.filesRepo.Delete(r.Context(), tx, *userID, rowID); err != nil {
		defer tx.Rollback()
		if errors.Is(err, entities.ErrDoesntExist) {
			w.WriteHeader(http.StatusNotFound)
			w.Write([]byte("not found"))
		...
		w.WriteHeader(http.StatusInternalServerError)
		msg := fmt.Errorf("failed to delete file: %v", err).Error()
		w.Write([]byte(msg))
		return
	}
}
